...
谁是卧底默认关闭，支持 4-11 人单卧底和 6-11 人双卧底。创建房间和开始对局使用公屏，其他玩家只能通过好友私聊报名；机器人通过好友私聊发送位置和词语，描述使用公屏 `#内容`。投票时请好友私聊怀疑对象编号，例如 `#A`，也可以发送 `#弃权`。词对会在发词前永久标记为已用，Web 只提供脱敏状态、开始当前房间和结束控制。完整玩法见 [谁是卧底](docs/undercover.md)。
...
`friend_delivery.fast_match: true` 会在左侧唯一完整昵称稳定匹配并点击后，只连续确认顶部标题不是“当前大厅”或“公共大厅/公开频道”，不再到聊天内容区二次查找目标昵称；其他非空标题（包括“陌生人消息”）均可放行。左侧未找到目标时，从最底部头像向最顶部头像拖动后重试，最多拖动两次；恢复当前大厅时使用相同头像位置反向向下拖动，最多两次回到列表顶部。设为 `false` 可恢复标题及聊天区目标昵称确认。
