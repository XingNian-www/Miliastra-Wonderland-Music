...
这两个仓库各自提供 `inference.onnx` 和 `inference.yml`。仓库不是预生成的 OpenVINO IR，但可以直接用 OpenVINO 2026.2.1 的 `ov.convert_model()`/`ov.save_model()` 转成当前配置要求的一对 XML/BIN。转换后的 IR 应作为可选 OpenVINO 后端模型，不替换默认 MNN 模型，直到对聊天截图做准确率和延迟回归。
...
```yaml
ocr:
  # 使用 OpenVINO 后端时可以省略这两个 MNN/PaddleOCR 路径
  det_model: null
  rec_model: null
  backend_priority:
    - openvino
...
发布配置不要提交 `.build` 路径或临时下载文件；应把经过回归测试的四个 IR 文件放到 OpenVINO 依赖包的 `models/` 下，并使用相对路径。OpenVINO 推理本身只读取 IR/BIN 和字符集。上面的配置不会隐式回退到 CPU，也不需要 MNN 模型；如果要保留 fallback，需显式把 `cpu` 加入 `backend_priority`，并随普通 EXE 包提供 MNN 模型。默认构建 `cargo build --release` 已包含 OpenVINO 适配器；OpenVINO 工作流仅手动转换并打包 runtime/IR 依赖，不编译或发布 OpenVINO 专用 EXE。`cargo build --release --no-default-features --features ocr-openvino` 仍可用于本地验证不包含 `ocr-rs`/MNN 的最小构建。
