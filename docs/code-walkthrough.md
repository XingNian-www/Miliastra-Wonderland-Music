...
项目是一个 Windows-only Rust 单二进制程序。主线程负责截图、检测 UI、扫描聊天；命令执行线程负责串行执行所有会操作游戏窗口的业务流程；播放监控线程负责观察 FeelUOwn 状态并在合适时机向同一条待执行任务队列塞入自动出队任务；HTTP/Web 面板只提交远程任务或读取监控状态，不直接做底层游戏输入。
...
- 没有子命令时进入常驻自动化模式。
- 有子命令时执行调试工具，例如 OCR 图片、OCR 区域、扫描聊天、检测 UI 状态、模板匹配、点击模板、发送聊天等。

常驻模式走 `run_automation_with_watchdog()`。外层父进程启动带 `--watchdog-child` 的子进程；子进程异常退出时，父进程按配置延迟后重启。真正业务在 `run_automation()`。
...
- `screen`：聊天区、Enter 模板区、大厅 OCR 区域等固定坐标。
- `timing`：按领域分组的延迟和超时。
- `ocr`：OCR 模型、后端优先级、聊天块切分参数。
- `templates`：聊天标记、Enter、好友/邀请/管理按钮模板。
- `output`：游戏内发言点击点。
- `feeluown`：播放器 RPC 地址。
...
- FeelUOwn 客户端、点歌 AI 客户端、候选歌曲审核客户端。
- 游戏聊天输出器。
- 共享 OCR 引擎。
...
窗口丢失时不会忙等，而是按退避时间重试。启动游戏任务、进入千星任务等会通过 `WindowDetectionSignal` 重置退避，让刚启动的窗口能更快被重新检测。
...
- `UserCommand`：点歌、暂停、下一首、队列、邀请、拉黑/屏蔽、麦克风、自定义工作流等业务命令。
...
主扫描循环只负责把解析出的命令交给 `CommandLockState`。通过屏幕锁的新命令会进入待执行任务队列。
...
执行前通常会调用 `prepare_command_ui()`：

1. 激活并聚焦游戏窗口。
2. 检测是否已经在一级界面。
3. 如果不是，按 ESC 返回上一级。
4. 超时则把任务放回队首。
5. 如果目标窗口不可用，则中止当前任务并输出明确错误。

## 播放监控和自动出队

播放监控线程在 `run_playback_monitor_loop()`。

更完整的音乐播放队列、点歌播放标记、临近结束暂停和自动出队链路见 `docs/playback-queue-flow.md`。

它定期查询 FeelUOwn 状态，并调用 `maybe_advance_queue()`。核心策略：

- 如果用户主动暂停，则不自动处理。
- 如果当前歌曲即将结束且有待执行点歌或队列，先暂停播放，避免当前歌结束后播放器自己继续。
- 如果队列非空且没有命令正在执行，把 `PendingTask::AdvanceQueue` 放入待执行任务队列。
- 如果之前因为待执行播放而暂停，但后来没有任务了，会恢复播放。
...
5. `WaitPrimaryEnterTemplate`：等待左下角全局 Enter 模板出现，认为开门完成。
