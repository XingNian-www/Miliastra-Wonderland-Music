...
普通娱乐命令和稳定海龟汤提问先按观察顺序进入正式任务队列，再由执行器更新 `BusinessRuntime` 状态或提交后续效果；这样后出现的玩法输入不会越过前面的控制命令。命令可用性开关、闲置退出和监听模式控制保留既有运维路径。需要发送的结果仍成为延迟聊天项、`CardGameEffect` 或 `UndercoverEffect`，再取得调度和 UI 顺位。
