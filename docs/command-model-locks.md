...
- 海龟汤 OCR 稳定锁：服务一级大厅中以 `#` 开头的普通问题、编号片段和提交动作。昵称与正文分别连续一致达到配置次数后才处理，任一项变化只重置自己的计数；启动首屏只建立基线。稳定后的身份键由玩家昵称、输入类型、批量编号和正文组成，比较时忽略空白、大小写、全半角和标点差异。问题还会辅助昵称去重：输入类型和问题相同、规范化昵称最多相差一个字符时视为同一条 OCR 消息；明显不同的昵称仍然独立。输入类型、编号或实际文字变化仍视为新输入。同编号片段即使再次进入业务层也只会覆盖，不会重复拼接正文。

它们都不是业务互斥锁，也不替代待执行任务队列。

二级聊天监听不复用一级消息位置锁：好友红点被首次进入时的未读清场消除，当前大厅按气泡序列重叠只处理新增气泡。海龟汤进行期间会额外对新增气泡的正文和昵称做跨轮连续一致校验；未稳定时不推进气泡基线。二级命令仍会检查待执行任务队列，稳定后的海龟汤提问直接进入独立 AI 队列。具体扫描和恢复规则见 [二级聊天监听](secondary-chat-listener.md)。
...
当聊天命令进入待执行队列时，`PendingCommand` 会同时保留产生它的观察上下文。上下文只存在于运行时，不改变 HTTP 或持久化协议，包含：

- `frame_id`：产生命令的观察帧编号。
- `captured_at`：该观察帧的截图捕获时间，期限判断使用它而不是 OCR 完成时间。
- `message_id`：消息在当前视觉会话中的稳定标识。

控制台提交的命令没有聊天观察来源，这三个字段保持为空。这样执行器、日志和业务模块可以关联原始观察证据，同时继续把命令本身作为唯一的业务输入。
