...
`src/app/web_tools.rs` 管理高级控制页的低优先级诊断任务。独立后台线程只在正式待处理队列为空且未执行正式命令时取出任务；失败会写入任务结果，不会中断主循环或触发业务恢复动作。`/tools` 页面提供 OCR、聊天扫描、UI/大厅识别、命名模板匹配、坐标点击、按键、OCR 后端探测与 AI 候选诊断。
...
海龟汤另有一份永久使用记录，默认路径是 `data/turtle-soup-used.json`。它只保存已经被选中的题目 ID，并使用临时文件加替换的方式原子写入。海龟汤当前会话仍是进程内状态，程序重启不会恢复；正式题库 `turtle_soup.yaml` 是只读输入，不属于状态文件。
