...
> 本文记录 2026-07 的 DeepSeek 专项历史调研。当前默认请求已统一为 OpenAI 标准字段，不再自动发送 `thinking`，配置也不再提供 `thinking_enabled`；确有第三方兼容需要时只能通过 `extra_body` 显式补充。下文相关示例仅用于解释旧评测条件，不代表当前默认实现。
...
3. 历史调研推荐 `deepseek-v4-flash` 并显式关闭思考模式。当前实现优先发送 OpenAI 官方字段，不再自动发送 DeepSeek 私有 `thinking`；需要复现该调研条件时，应在 `turtle_soup.ai.extra_body` 中显式配置 `thinking.type=disabled`。[来源 1][来源 4]
...
当前实现按 OpenAI Chat Completions 标准字段发送以下结构；不会自动添加 DeepSeek 私有 `thinking`：
...
`max_tokens` 是标准配置项；`response_format`、`temperature`、`stream: false` 和 `store: false` 由代码按 OpenAI 协议固定。若 DeepSeek 模型需要显式关闭思考，可额外配置：
...
`extra_body` 只承载这类第三方兼容字段；若其中出现与标准请求同名的字段，代码生成的 OpenAI 官方字段优先。

上面的 `1024` 是当前仓库默认配置。历史评测曾使用 `max_tokens: 256`，该数值只用于复现旧评测，不应当作为当前配置文档或部署默认值。
...
- 输出上限已使用官方 `max_tokens` 字段，数值由 `turtle_soup.ai.max_tokens` 配置，当前默认 `1024`。
- 历史 553 次评测使用过 `256` 个输出 Token；它对当时的单字段裁决 JSON 足够，但只应作为历史实验参数，不代表当前默认值。
...
API Reference 建议调整 `temperature` 或 `top_p` 中的一项，不要同时调整。当前实现固定发送 `temperature: 0.0`，不发送 `top_p`；它不会根据 DeepSeek 私有思考模式改写标准请求。[来源 3][来源 4]

DeepSeek 思考模式不支持 `temperature` 和 `top_p`，服务端为兼容现有软件可能接受它们但不会生效。[来源 4] 当前实现不会因 `extra_body.thinking` 改写或删除 OpenAI 标准字段；选择第三方思考模式前，应先确认 Provider 能接受这套标准请求契约。
...
| 思考模式 | 默认不发送 `thinking`；可由 `extra_body` 显式兼容 | 历史评测条件为 `thinking.type=disabled` | 按需配置 |
| JSON Output | 已发送 `json_object` | 保持 | 已符合 |
| JSON 提示 | 已含 `JSON` 和目标格式 | 保持 | 已符合 |
| 输出上限 | 由 `max_tokens` 配置，默认 `1024` | 历史评测曾使用 `256`；思考模式按需提高 | 已符合 |
| 随机性 | 固定 `temperature: 0.0`；不发送 `top_p` | 保持标准请求 | 已符合 |
