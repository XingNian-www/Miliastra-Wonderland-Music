...
本文梳理远程指挥台的代码边界：Web 页面怎样加载，HTTP API 怎样分发，哪些接口只读，哪些接口提交正式任务或模块化 mutation，以及监控快照从哪里来。
...
少数接口不创建正式 UI 任务：`/queue/add`、`/player/play-uri` 和 `/state/save` 通过 `HttpTaskPort` 提交类型化业务 mutation。它们仍由业务运行时串行修改所属模块；前两个修改音乐播放队列，`/state/save` 只修改大厅倒计时等受限大厅状态。`/player/play-uri` 不直连 FeelUOwn，也不把播放器标记为外部播放。
...
flowchart TD
    ...
    C -->|远程命令| E["ConsoleCommandIntent + ModuleCommand"]
    C -->|直接任务| F["HttpTaskPort 类型化提交"]
    C -->|模块 mutation| G["BusinessMutationIntent<br/>Administration / Playback / TurtleSoup"]
    E --> H["待执行任务队列"]
    F --> H
    G --> J["业务运行时模块所有者"]
    H --> I["业务运行时正式调度通道"]
...
| `src/interfaces/http/tools.rs` | 高级工具请求类型、参数解析、标签与配置模板映射。 |
| `src/runtime/scheduler.rs` | 正式业务与诊断通道的任务 ID、生命周期、容量和结果摘要。 |
| `src/runtime/decision.rs` | 把 Web 候选决策送回正在等待的点歌确认流程。 |
| `src/features/playback/queue.rs` | 持久播放队列、稳定队列项 ID、当前格式校验和原子保存。 |
...
这些接口直接构造 `ConsoleCommandIntent`，其中包含所属 feature 的类型化 `ModuleCommand`，再通过 `HttpTaskPort` 提交。HTTP 不生成命令信封，也不拼接伪聊天文本：
...
海龟汤控制向业务运行时提交 `TurtleSoupMutationIntent`，不经过聊天命令路由，也不直接操作游戏输入。业务运行时更新会话、题库使用记录和 AI 队列；汤面、裁决与结算仍进入延迟聊天通道，并按正式调度规则发送。
...
| --- | --- |
| `queued` | 仍在正式任务队列中等待。 |
| `running` | 已被正式任务调度器取出，正在执行。 |
| `completed` | 正常完成。 |
| `failed` | 执行返回错误或发生未捕获异常。 |
| `canceled` | 在开始前被 Web 控制台撤销。 |

任务记录同时包含入队、开始、结束时间、执行耗时和结果摘要。`POST /tasks/cancel?id=...` 只能移除 `queued` 任务；任务一旦被正式任务调度器取出就返回 `409`，不会尝试中断正在操作游戏窗口的流程。
...
Web 提交决策与游戏聊天决策共用同一个等待会话。等待期间共享聊天观察仍继续发布普通命令；一级和二级监听按各自的屏幕/气泡基线及待执行队列去重，决策回复不会被普通命令路由或自定义工作流重复消费。

### 队列和状态变更

这些接口不创建聊天命令或 UI 正式任务，而是直接提交所属模块的类型化 mutation：

| 接口 | 行为 |
| --- | --- |
| `/queue/add` | 提交播放模块队列 mutation，由业务运行时写入并同步监控快照。 |
| `/player/play-uri` | 提交包含 `fuo://` URI 的控制台高权限队列 mutation，由业务运行时写入并同步监控快照。 |
| `/queue/remove` | 提交按稳定队列项 ID 删除的播放模块 mutation。 |
| `/queue/clear` | 提交清空音乐播放队列的播放模块 mutation。 |
| `/state/save` | 提交有限字段大厅状态 patch，由 Hall 模块所有者校验并保存；不能修改播放器确认状态、暂停原因或活动播放请求。 |

`/queue/add` 和 `/player/play-uri` 是控制台最高权限入口，不做候选歌曲审核。它们适合人工明确知道要塞什么队列项的场景；写入后仍由播放应用服务在正式出队时执行 URI 播放和稳定确认。

每个播放队列项都有持久 `id`。队列文件保存当前格式的 `nextId` 和 `items`，自动播放移除队首后，其他歌曲的 ID 不会改变。Web 面板删除歌曲使用 `/queue/remove?id=...`；旧的 `index` 参数仅保留给既有调用方，不建议用于会并发自动出队的界面。

播放器状态、暂停原因、活动播放请求和最近播放观测由播放器控制器维护，不能通过 `/state/save` 直接改写。`/state/save` 的状态补丁实际调用 `HallMutationIntent::PatchState`，不会保存通用播放器状态。
...
| `turtleSoup` | 业务运行时发布的不含汤底的海龟汤快照。 |
| `undercover` | 业务运行时发布的不含秘密内容的谁是卧底快照。 |
...
`pendingTasks` 是兼容用的当前等待任务标签；`tasks` 是正式任务生命周期；`webTools` 是独立高级工具任务；`decision` 是当前点歌候选决策，没有等待决策时为 `null`。海龟汤和谁是卧底快照已经属于投影本身，不再由 Web 请求额外查询业务运行时。
...
- 业务运行时状态变化时提交 `MonitorEvent::TurtleSoup` 和 `MonitorEvent::Undercover`。
- 日志输出时由 `MonitorLogSink` 提交 `MonitorEvent::Log`。

所有事件由 `MonitorProjection` 单一投影入口应用，生产模块不直接修改快照。

HTTP 队列、海龟汤、监听模式和运行时状态变更都会先转换为类型化业务变更意图。业务运行时完成变更后立即向 `MonitorProjection` 发布对应事件，因此 Web 面板不需要等待下一轮业务流程才看到新状态。

HTTP 只通过显式任务端口和只读查询端口访问业务运行时；取消任务、提交决策、读取播放队列、运行状态及娱乐模块快照都不需要把完整 `BusinessRuntimeHandle` 暴露给协议层。
...
截图保持原始尺寸，不进入常驻监控刷新。主扫描尚未取得首帧或最近一次截图已因目标窗口不可用而失效时返回 `503`，不会继续提供关闭游戏前的旧画面。前端会创建临时 object URL，关闭弹窗时释放。
