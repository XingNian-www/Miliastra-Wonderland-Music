...
点歌不是 HTTP 或 OCR 入口直接播放。所有常规点歌都会先变成 `UserCommand::Song`，进入待执行任务队列，再由命令执行线程串行处理。处理过程中会先得到最终候选歌曲，再做候选歌曲审核，然后才决定直接播放还是加入音乐播放队列。长时间同歌去重不在入队时计数，只在实际播放前检查，确认播放成功后才记录历史。
...
2. `feeluown.search_and_pick()` 搜索并挑出一个候选。
...
1. 回复 `AI匹配中`。
2. `feeluown.search_candidates()` 获取候选列表。
...
这一步只避免同一首候选在音乐播放队列里重复排队，不代表长时间同歌去重。长时间同歌去重保护的是实际播放时刻，入队时不检查、不计数。
...
- 没有保护条件且可以立即播放时，先检查长时间同歌去重，通过后调用 `play_request_confirmed()`。

当前歌曲保护由 `should_queue_until_current_song_finished()` 判断。配置关闭时可以更积极地直接播放；配置开启时，正在播放或可识别的暂停歌曲会优先保留，新点歌进入音乐播放队列。

## 实际播放确认

`play_request_confirmed()` 进入播放器前会先调用 `song_dedup_limited()` 检查近期实际播放历史。控制台来源在 `song_dedup.console_bypass = true` 时跳过这一步；其他来源超限会回复 `歌曲名近期已播放过,请稍后再点`，本次点歌结束。

通过长时间同歌去重后，`play_request_confirmed()` 根据请求是否有 URI 分两种：

- 有 URI：`play_uri_confirmed()` 直接 `feeluown.play_uri()`。
- 没有 URI：`play_keyword_confirmed_inner()` 再次按关键词搜索并播放。

两条路径都会进入 `confirm_playback_started()`。它会反复查询 FeelUOwn 状态，并做几层确认：

1. 播放状态必须是 `playing` 或 `paused`。
2. 有 URI 时优先确认当前 URI 是否等于请求 URI。
3. URI 不一致时，普通点歌会用本地歌曲匹配和 AI 同曲判断兜底。
4. 匹配失败会让用户 `@确认@跳过`，允许时还可以 `@换源`。
5. 进度和时长不能是无效的 `0:00/0:00`。
6. 时长过短会视为无音源。
7. 成功后记录点歌播放标记，写入长时间同歌去重历史，并回复 `播放: 歌名 - 歌手 (进度/时长) 音量x`。

点歌播放标记写在运行状态里，用于后续判断当前歌曲是否仍是本次点歌以及自动出队是否应该暂缓。长时间同歌去重历史写在 `song_dedup.history_path`，只记录确认播放成功的歌曲。

## 自动出队

播放监控线程只观察 FeelUOwn 状态，不直接播放下一首。它在合适时机提交 `PendingTask::AdvanceQueue`：

- 播放器停止，且音乐播放队列非空。
- 播放器暂停并接近结束，且音乐播放队列非空。
- 播放器正在播放并接近结束，且存在待执行播放工作；此时可能先暂停，避免播放器自然跳到非队列歌曲。
...
`@下一首` 如果音乐播放队列非空，会直接调用 `consume_queue("手动下一首")`，优先播放队列里的下一项。队列为空时才调用 FeelUOwn 的 `next`。
...
- 控制台免审不等于控制台直连播放。
- 长时间同歌去重只保护实际播放时刻，不拒绝入队；只有播放成功才写历史。
