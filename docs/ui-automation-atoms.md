...
| 文件 | 职责 |
| --- | --- |
| `src/app/workflow_actions.rs` | 原子动作集合：等待、按键、激活、聚焦、点击点、粘贴、模板等待/点击、文字等待/点击、像素稳定。 |
...
`focus_point` 只在显式 `focus_game` 或命令执行前准备阶段使用。普通原子动作不自动点击 `focus_point`。
...
UI 状态检测优先检查 Enter 模板，再检查大厅二级界面模板，最后才数聊天标记。这个顺序能让返回一级界面在看到 Enter 按钮时尽早成功。
...
3. 如果是 `primary:enter` 或 `primary:marker`，认为已经回到一级界面。
4. 如果是大厅二级或未知界面，按 ESC 返回上一级。
5. 连续失败时增加等待时间；超过 5 次后固定 2000ms。
6. 如果目标窗口不可用，立即停止并返回失败。

`return_to_primary_from_transient_ui()` 会先强制按一次 ESC，适合从临时弹窗或聊天窗口退出。`return_to_primary_fixed()` 不强制首个 ESC，适合命令前检查当前是否已经在一级界面。

## 命令前准备

命令执行线程在业务任务前调用 `prepare_command_ui()`：

1. `ensure_game_ready_for_input()` 激活并聚焦游戏。
2. 检测当前 UI 状态。
3. 如果不是一级界面，按 ESC 返回。
4. 在超时内仍无法回到一级界面，则当前任务会保留或重排，而不是继续盲目执行。

这层保证了大多数业务流程开始时处于可预测界面。
...
- 原子动作默认游戏已经聚焦；聚焦由命令前准备或显式 `focus_game` 完成。
