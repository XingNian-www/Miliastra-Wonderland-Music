...
|     `- song_request_port.rs      # 点歌应用服务到组合能力的窄适配
|- config/
...
|  |- ui.rs                        # 游戏窗口单一所有者
|  |- ocr.rs                       # OCR 引擎单一所有者
|  |- player.rs                    # 播放器观测模型与稳定证据
|  |- player_io.rs                 # 观测、控制、搜索三条运行时通道
|  |- timer.rs                     # 单调期限
|  `- monitor.rs                   # 监控投影
|- ui/
|  |- frame.rs                     # 截图 Frame 与画布归一化
|  |- state.rs                     # 单帧分类与稳定跟踪
|  |- template.rs                  # 纯模板匹配
|  |- locator.rs                   # 无 I/O 的大厅信息解析与区域计算
|  |- atoms.rs                     # 原子动作
|  `- routines/                    # 独立类型化 UI 例程
|- observation/
|  |- chat.rs                      # 聊天切块、稳定消息与观察标识
|  |- shared.rs                    # 有界共享观察流和缺口
|  `- exclusive.rs                 # 独占决策读者会话
|- features/
|  |- playback/                    # 播放应用服务、控制器、队列、去重和确认
|  |- song_request/                # 点歌应用服务、候选契约、AI 与审核
|  |- startup.rs                   # 启动游戏与进入千星
|  |- invite.rs                    # 邀请与好友反馈
|  |- moderation.rs                # 管理投票与执行
|  |- custom_workflow.rs           # 唯一原子动作计划解释器
|  |- idiom_chain.rs
|  |- card_games.rs                # 斗地主与跑得快共享牌型能力
|  |- undercover/                  # 谁是卧底服务与题库仓库
|  `- turtle_soup/                 # 海龟汤服务与题库仓库
|- adapters/
|  |- windows/                     # 窗口、输入、截图、剪贴板
|  |- feeluown.rs                  # 单次原始 RPC
|  |- player.rs                    # 播放器运行时到播放业务端口的适配
...
- OCR 不负责截图、区域选择、稳定策略或业务秘密；`OcrRuntime` 只拥有识别设备和请求队列，调用方负责图块和语义。
- HTTP 不直接修改业务状态，不伪造聊天命令，也不持有完整业务运行时句柄。
- HTTP 修改意图的顶层枚举只区分纵向模块，具体操作与结果由模块自有类型定义。
- 监控快照不是业务状态仓库；它由单一投影线程顺序应用类型化事件，TUI/Web 只读快照。
...
- 播放、点歌、大厅、管理、成语、海龟汤、牌局和谁是卧底的应用流程由各自纵向模块拥有；组合层只实现窄能力端口、聊天观察适配和任务装配。
- 只有能够分发全部纵向模块的正式任务执行器持有完整组合对象；其他后台线程和即时路径必须使用按职责构造的窄工作上下文。
...
- 正式任务、延迟发送和诊断请求保留三类调度通道；诊断任务只有在正式任务空闲时才可取得执行 lease。
...
- 管理投票等动态后台线程由应用生命周期登记并在业务、UI 和 OCR 运行时停止前等待结束。
...
5. HTTP 内部不再伪造聊天文本或直接修改共享状态，但外部协议保持兼容；控制台命令在边界构造 `ConsoleCommandIntent`，不经过视觉屏幕锁。
...
6. **聊天观察**：迁移一级和二级监听、共享命令流、独占决策读者、消息标识和观察缺口。
