...
- `BusinessRuntime` 独占正式调度器、娱乐状态、播放/大厅状态、播放队列、监听状态和延迟发送队列；`MonitorShared` 独占监控投影。
...
1. 从工作目录读取 `config.yaml`，先通过 `ResolvedApplicationConfig::resolve` 完成组合校验和窄配置解析。
2. 创建 `MonitorShared`，再按依赖顺序构造 OCR、播放器、UI、OpenAI 和业务运行时。
3. 在 `ApplicationRuntime` 内加载播放状态、大厅状态、播放队列、去重历史和词库等持久数据。
4. 启动正式任务执行器、HTTP/TUI/热键和观察循环。
5. 关闭时先停止观察与业务线程，再有序关闭 UI、OCR、播放器、OpenAI、监控投影并保存持久状态。
...
- `state.queue_path`
- `song_dedup.history_path`
- `undercover.word_bank_path` / `undercover.used_state_path`
- `turtle_soup.question_bank_path` / `turtle_soup.used_state_path`
...
候选确认、邀请确认和管理投票使用独占决策读者建立自己的消息基线，但不会暂停共享聊天命令流。一级普通命令继续经过 `CommandLockState`，二级普通命令依靠气泡序列/消息基线，二者都再经过正式任务队列去重；`@确认`、`@跳过`、`@换源`、`@AI` 及邀请/投票回复由静态路由器保留给决策读者。
...
- `src/runtime/ocr/engine.rs` 拥有 OCR 模型与后端选择；默认 EXE 同时包含 MNN 与 OpenVINO IR 适配器，按优先级在运行时选择。OpenVINO runtime DLL 只在选择该后端时加载；用 `--no-default-features --features ocr-openvino` 仍可构建不包含 PaddleOCR/MNN 代码的 OpenVINO-only 二进制。
...
- 普通聊天输入先按观察顺序进入正式队列，再更新业务状态或产生效果。
- 短回复进入 `DeferredChatQueue`。
- 好友验证、发牌和完整阶段 UI 事务保留在 Formal 通道；输入排队与效果通道分别决定。
- 海龟汤 AI worker 只调用模型并回传带会话代数的结果；业务 runtime 决定是否仍可应用。
- 娱乐期限使用共享 `Clock`/deadline 语义；聊天输入携带截图捕获时间，测试通过 `ManualClock` 推进，不依赖真实睡眠。
...
- 海龟汤使用独立 Provider 配置。
- 仓库不包含云崽或其他 Node 运行时代码；外部插件若接入主程序，需自行遵守 OpenAI 标准字段和鉴权边界。
...
`src/runtime/monitor.rs` 通过单一投影线程汇总日志、OCR、调度、播放、UI 状态、监听状态、诊断任务和娱乐公开快照供 TUI/Web 读取。HTTP 协议尽量保持兼容，但内部类型、旧配置和旧命令实现不作为兼容面。
