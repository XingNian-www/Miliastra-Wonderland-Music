...
3. 检测到游戏窗口后聚焦游戏窗口一次
4. 在 `startup.enter_game_text_region` 循环 OCR，最多 60 秒等待“点击进入”四个字
5. 识别到“点击进入”后点击文本框中心，并持续点击直到该文字不再出现
6. 文字消失后等待全局 `templates.enter` 在 `screen.enter_rect` 中出现，出现后认为启动游戏任务完成

进入千星任务流程：

1. 查找并聚焦已有游戏窗口；找不到窗口时直接失败，需要先执行启动游戏任务
2. 周期性按 `F6`，直到在右上角区域检测到千星奇域主页的 `wonderland_close` 模板
3. 点击配置的第一个奇域卡片坐标，等待黑色确认按钮模板在确认区域出现
4. 点击黑色确认按钮，并等待确认按钮消失
5. 确认按钮消失后最多等待 20 秒，继续在进入后确认区域检测黑色确认按钮模板；再次出现就认为已经进入千星
...
去重只在实际播放前检查：直接播放前检查一次，音乐播放队列出队准备播放前检查一次；点歌加入队列时不检查、不计数。只有确认播放开始成功后才会写入 `history_path`，所以搜索失败、入队、审核拒绝、取消确认都不会污染历史。

超出限制时，直接播放会在大厅回复 `歌曲名近期已播放过,请稍后再点`；队列出队会移除该项并回复 `歌曲名近期已播放过,已跳过`，然后继续尝试下一首。控制台来源默认豁免，可以通过 `song_dedup.console_bypass` 关闭。
...
内置好友发言、邀请和 UID 管理流程也使用同一套原子动作：任务入口会显式激活并聚焦游戏一次，后续点击、按键、粘贴默认游戏已经可接收输入；返回一级界面只使用 ESC 逐级返回。等待 OCR 文字时会轮询到超时，等待模板时会轮询到出现或消失，打开好友聊天后会等待聊天区域像素稳定再发送文本。
