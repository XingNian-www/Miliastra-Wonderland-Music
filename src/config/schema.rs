...
impl ConfigFieldSchema {
    /// 数据库可编辑、重启生效字段。
    fn db_restart(path: &str, label: &str, kind: FieldKind, hint: &str) -> Self {
        Self {
            path: path.to_string(),
...
fn stability_section() -> Vec<ConfigFieldSchema> {
    vec![
        ...
        ConfigFieldSchema::db_restart(
            ...
            "二级大厅确认次数",
            int(0, MAX_STABLE_COUNT),
            "二级大厅气泡与发送者关联的局部覆盖；大于 1 时优先，否则继承 default_count",
        ),
    ]
}
...
fn timing_section() -> Vec<ConfigFieldSchema> {
    vec![
        ...
        ConfigFieldSchema::db_restart(
            ...
            "主循环空转间隔",
            int(1, MAX_TIMEOUT_MS),
            "监听主循环空转间隔；脚本暂停或每轮扫描结束后等待多久再继续，单位毫秒",
        ),
        ConfigFieldSchema::db_restart(
            "chat_scan.fallback_ms",
            "聊天兜底扫描间隔",
            int(1, MAX_TIMEOUT_MS),
            "聊天 OCR 兜底扫描间隔；画面没变化时按这个间隔强制扫描一次，单位毫秒",
        ),
        ConfigFieldSchema::db_restart(
            "chat_scan.change_debounce_ms",
            "变化去抖等待",
            int(1, MAX_TIMEOUT_MS),
            "聊天变化后等待画面稳定再 OCR 的时间，单位毫秒",
        ),
        ConfigFieldSchema::db_restart(
...
fn ocr_section() -> Vec<ConfigFieldSchema> {
    vec![
        ...
        .nullable(),
        ConfigFieldSchema::db_restart("charset", "字符集", FieldKind::Path, "PaddleOCR 字符集路径"),
        ConfigFieldSchema::db_restart(
            "min_confidence",
            "最低置信度",
            float(0.0, 1.0),
        ...
        ConfigFieldSchema::db_restart("threads", "OCR 线程数", int(1, 1024), "OCR 线程数"),
        ConfigFieldSchema::db_restart(
            "request_timeout_ms",
            "请求超时",
            int(1, MAX_TIMEOUT_MS),
...
fn queue_section() -> Vec<ConfigFieldSchema> {
    vec![
        ConfigFieldSchema::db_restart("max_size", "队列最大长度", int(1, 100_000), "队列最大长度"),
        ConfigFieldSchema::db_restart(
            "pool_max_size",
            "播放池容量",
            int(0, 1_000_000),
...
fn hotkeys_section() -> Vec<ConfigFieldSchema> {
    vec![
        ConfigFieldSchema::db_restart("enabled", "启用热键", FieldKind::Bool, "是否启用全局热键"),
        ConfigFieldSchema::db_restart(
            "pause_key",
            "暂停/恢复热键",
            FieldKind::String,
        ...
        ConfigFieldSchema::db_restart("exit_key", "退出热键", FieldKind::String, "退出热键"),
    ]
}
...
fn custom_workflows_section() -> Vec<ConfigFieldSchema> {
    vec![
        ...
        ConfigFieldSchema::db_restart(
            ...
            "模板消失后等稳定",
            FieldKind::Bool,
            "wait_template_absent 默认在模板消失后继续等待像素稳定；可在步骤上设置 stable_after_absent: false 关闭",
        ),
        ConfigFieldSchema::db_restart(
            "max_hold_key_seconds",
            "按住按键上限",
            int(1, 86_400),
            "hold_key 单次按住按键允许的最大秒数；时长从命令参数读取",
        ),
        ConfigFieldSchema::db_restart(
            "templates",
            "自定义模板映射",
            FieldKind::Object,
            "自定义模板名到图片路径的映射，步骤里通过 template 引用这里的名字（高级 JSON 编辑）",
        ),
        ConfigFieldSchema::db_restart(
            "workflows",
            "自定义流程定义",
            FieldKind::Object,
            "自定义流程定义列表；支持的步骤与变量见配置模板注释（高级 JSON 编辑）",
        ),
    ]
}
