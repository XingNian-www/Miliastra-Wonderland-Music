...
- 命令由单一业务运行时调度；UI、OCR 和播放器分别由独立运行时拥有，等待外部结果时聊天观察仍会持续运行
...
3. 检测到游戏窗口后聚焦游戏窗口一次
4. 每轮先匹配 `startup.templates.paimon_menu` 在 `startup.main_ui_region` 中是否已出现；出现即认为已经进入一级界面并完成任务
5. 未出现时才在 `startup.enter_game_text_region` 循环 OCR，最多 60 秒等待“点击进入”四个字
6. 识别到“点击进入”后点击文本框中心，并持续点击直到该文字不再出现
7. 文字消失后等待 `startup.templates.paimon_menu` 出现，出现后认为启动游戏任务完成
...
黄色聊天标志只用于辅助切分聊天块，不作为命令来源。`@帮助` 只展示点歌帮助；`#娱乐` 或 `@娱乐帮助` 展示娱乐玩法命令。
...
二级监听不使用一级聊天的命令屏幕锁。当前大厅映射为 `blue`，通过可见气泡序列重叠识别并按顺序处理本轮全部新增的他人深色气泡；好友会话映射为 `pink`，每个未读好友只读取最下方最新消息。左侧好友未读红点只扫描当前可见区域；首次进入会逐个清掉可见旧红点而不执行命令，之后每次只处理一位好友并返回当前大厅监听一轮。陌生人消息不会触发命令；公开频道会尝试切回当前大厅，失败后退回一级监听。
...
点歌确认、邀请确认和管理投票等待期间，独占读者只读取自己的决策基线和决策语法；聊天观察不会暂停。一级普通命令继续经过命令屏幕锁，二级普通命令继续经过气泡序列/消息基线，两者都在待执行队列中做语义去重。`@确认`、`@跳过`、`@换源`、`@AI` 以及邀请/投票回复是保留决策语法，不会被自定义工作流抢占。
...
| `@网易点歌 歌名 歌手` / `@网易云点歌 歌名 歌手` / `@网易搜索 歌名 歌手` | 强制使用网易云音乐 |
| `@暂停` | 暂停播放 |
| `@继续` / `@恢复` / `@播放` | 继续播放 |
| `@下一首` / `@下一曲` | 播放下一首 |
| `@上一首` / `@上一曲` | 播放上一首 |
...
| `@状态` | 查询当前播放状态 |
| `@歌词` | 查询当前歌词 |
| `@歌词 1-30` | 在指定秒数内自动输出变化的歌词；歌曲切换后提前结束 |
| `@持续歌词` | 主线持续输出变化的歌词；遇到正式命令立即结束并让正式命令继续执行 |
| `@后台歌词` / `@停止歌词` | 后台监听歌词；正式命令执行期间暂不发送，完成后自动继续；用停止命令关闭 |
| `@队列` / `@列表` | 查看点歌队列 |
| `@完整队列` / `@完整列表` | 不压缩显示完整点歌队列；按聊天长度批量发送，能合并到一条消息的歌曲会放在同一条 |
| `@队列删除 1` | 删除队列第 1 首。支持写多个数字，如 `@队列删除 13` |
| `@队列清空` | 清空队列 |
| `@大厅检测` | 打开 F2 大厅页识别当前大厅 |
| `@大厅时间` | 回复已识别到的大厅剩余时间 |
| `#接龙 成语` | 开始或继续严格同字成语接龙 |
| `#同音接龙 成语` | 开始忽略声调的同音接龙，后续仍可用 `#接龙 成语` |
| `#提示` / `#解释 [成语]` | 推荐不会立即封龙的可接成语；查看指定成语或当前成语的来源与解释 |
| `#海龟汤` / `#状态` / `#结束` | 开始或重发海龟汤；进行中查看状态或结束对局 |
| `#问题内容` | 在进行中的海龟汤里提交 AI 裁决问题，例如 `#他认识死者吗` |
| `##编号内容` / `##提交` / `##取消` | 按编号暂存超过聊天长度的海龟汤答案，完整合并、提交或取消 |
| `#卧底` / `#卧底双` | 创建单卧底或双卧底报名房间 |
| `#开局` / `#状态` / `#退出` / `#结束` / `#重试` / `#内容` | 谁是卧底公屏开局、查询、退出、结束、重试或提交描述 |
| `#加入` / `#退出` / `#弃权` / `#A` / `#投A` / `#谜底` | 谁是卧底好友私聊报名、退出、弃权、投票或局外查询答案 |
| `#娱乐` / `@娱乐帮助` | 发送娱乐玩法帮助 |
| `@帮助` | 只发送点歌帮助 |
...
海龟汤默认关闭，只在当前大厅接收以 `#` 开头的提问，并与成语接龙全局互斥。题目选中后会先永久写入已使用记录，再分段发送汤面；汤面全部发完后才开始计时和接收提问。完整还原需要两次独立 AI 裁决都返回“完全正确”。长答案可以依次发送 `##1第一段`、`##2第二段`，最后发送 `##提交`；也可以发送 `##取消` 清空当前昵称的暂存内容。机器人只短回复 `暂存[昵称]:##编号`，提交时按编号合并成一次 AI 裁决。普通裁决会附带问题摘要，例如 `[星念]的男人是灯....理员吗？回复：否`。昵称和正文分别需要连续 OCR 一致后才会处理，默认各为 2 次，可通过 `turtle_soup.nickname_stable_count` 和 `turtle_soup.content_stable_count` 调整；问题相同且昵称只有一个规范化字符差异时按同一条 OCR 消息去重。

正式题库使用被 Git 忽略的 `turtle_soup.yaml`，格式参考 `turtle_soup.example.yaml`。Web 面板可以随机开局、按未使用 ID 开局、查看状态和主动结束，但不能查看进行中的汤底、编辑题库或重置使用记录。外部内容管理客户端只负责整理和审查内容，再通过 `POST /turtle-soup/questions` 提交最终题目；该客户端不属于本 Rust 仓库的运行时。详细规则见 [海龟汤](docs/turtle-soup.md)。

## 谁是卧底

谁是卧底默认关闭，支持 4-11 人单卧底和 6-11 人双卧底。创建房间和开始对局使用公屏，其他玩家只能通过好友私聊报名；机器人通过好友私聊发送位置和词语，描述使用公屏 `#内容`。投票时请好友私聊怀疑对象编号，例如 `#A`，也可以发送 `#弃权`。对局中 `#状态` 输出 `单卧底,第1轮,发言,Abcd` 这类紧凑状态，小写表示本轮未发言，大写表示已发言；投票阶段列出的玩家均已完成发言。对局外好友可私聊 `#谜底` 查看 `平民词:XXX,卧底词:YYY,卧底:AB`；对局玩家（包括淘汰者）会被拒绝，查询不推进牌局或计时。词对会在发词前永久标记为已用，Web 只提供脱敏状态、开始当前房间和结束控制。完整玩法见 [谁是卧底](docs/undercover.md)。

## 长时间同歌去重

`song_dedup` 用实际播放历史保护同一首歌的短时间重复播放。它按最终候选歌曲判断，不按原始点歌文本判断；只有双方都有非空 URI 且 URI 完全相同才视为同一首歌。缺少 URI 时不会用歌名、歌手或跨平台相似度兜底；伴奏和原唱也不会在没有 URI 时被错误合并。
...
等待候选决策时，其他普通聊天命令仍按当前一级或二级监听规则继续识别和入队；确认回复只由当前决策读者消费。
...
| `@网易点歌 歌名 歌手` / `@网易云点歌 歌名 歌手` / `@网易搜索 歌名 歌手` | 好友私聊网易云音乐点歌 |
| `@B站点歌 关键词` / `@B站搜索 关键词` | 好友私聊 B 站源点歌 |
| `@邀请1` / `@邀请654321` | 邀请机器人前往该好友大厅；1-3 位数字表示防冲突序号，6 位数字表示大厅密码并会在最后一步键盘输入 |
| `@麦克风` | 按 `N` 切换麦克风状态；当前在公共大厅时跳过 |
| `@监听模式 一级` / `@监听模式 二级` / `@监听模式 状态` | 切换或查询聊天监听模式；大厅蓝色消息不能使用 |
| `@W/@S/@A/@D [秒数]` | 在一级界面先按一次中键，再按住对应方向键；默认 1 秒，最大时长由 `custom_workflows.max_hold_key_seconds` 控制 |
| `@C` | 在一级界面按一次 `Ctrl` |
| `@CW/@CS/@CA/@CD [秒数]` | 依次按 `Ctrl`、中键、按住对应方向键、`Ctrl`；时长规则与 `@W/@S/@A/@D` 相同 |
| `@F` / `@X` | 在一级界面发送一次 `F` 或 `X`，完成后按当前监听模式恢复驻留 |
| `@通告 内容` | 向当前大厅发送指定内容 |
...
同意或默认同意时，机器人会把发起者昵称、私聊反馈、可选密码和最终驻留目标一次提交给完整邀请事务。该事务在同一次 UI 独占期间确认唯一好友和当前会话、发送反馈、稳定点击当前会话中最下方的左侧好友头像，再依次匹配邀请页面按钮并恢复监听驻留；反馈失败会作为警告保留并继续邀请，已经确认进入大厅的邀请不会因收尾恢复失败而重放。拒绝邀请使用普通好友投递事务发送反馈。

好友投递统一使用左侧好友列表中的唯一完整昵称进行稳定匹配；点击后顶部标题只用于拦截“当前大厅”和“公共大厅/公开频道”，不参与好友昵称匹配。左侧未找到目标时，从最底部头像向最顶部头像拖动后重试，最多拖动两次；恢复当前大厅时使用相同头像位置反向向下拖动，最多两次回到列表顶部。
...
| `@启用` | 恢复非粉色命令识别 |
| `@闲置退出` | 设置 30 分钟无新命令后暂停播放器、关闭目标游戏进程，软件主进程继续运行 |
| `@闲置退出 20` / `@闲置退出 20分钟` | 设置闲置退出时间，最小值为 15 分钟；触发时同样会先暂停播放器 |
...
1. 机器人在大厅通告本次请求
2. 后台等待粉色好友私聊投票，不阻塞普通点歌、状态、切歌等命令；普通命令继续进入共享命令流并按当前监听模式去重入队
3. 好友发送 `@同意` 或 `@不同意`
...
`config.yaml` 的 `custom_workflows` 可以添加配置驱动的大厅或好友命令。内置命令优先于自定义命令；确认、跳过、换源、邀请确认/拒绝和管理投票回复属于保留决策语法，即使写入 `commands` 也不会被自定义流程认领。其他自定义命令不带参数时能直接匹配，需要参数时给流程设置 `allow_args: true`
...
| `commands` | 触发命令列表，写命令名本身即可，例如 `测试流程`，聊天里使用 `@测试流程`；设为 `[]` 时只能按流程名从 Web 执行，不能由聊天触发 |
...
| `template` | 模板别名或图片路径 |
| `key` | 按键名，用于 `key/press_key`，支持 `Ctrl`、`Enter`、`Esc`、`F1` 到 `F12` 和单字符按键 |
| `button` | 鼠标按钮名，用于 `mouse_button`，只接受 `left`、`middle`、`right` |
...
| `hold_key` | 按命令参数指定的秒数按住按键 |
| `click` | 点击固定坐标 `point` |
| `mouse_button` | 在当前鼠标位置点击 `left`、`middle` 或 `right` 按钮；当前鼠标必须位于游戏窗口内 |
...
| `ensure_primary` | 检测并通过 ESC 逐级到达一级界面 |
...
原子动作只供自定义工作流机械段和 UI 内核内部组合。内置好友投递、邀请和 UID 管理分别使用独立的类型化 UI 事务；每个事务会在同一次 UI 独占期间观察并归一化起点、完成目标动作、确认结果并恢复请求指定的监听驻留。OCR、模板和像素稳定等待都在事务内部完成，结果会分别报告目标动作效果和最终驻留状态，避免收尾失败导致重复发送、重复邀请或重复管理操作。
...
| `/undercover` | 查看脱敏的谁是卧底状态 |
| `/undercover/start` | 开始当前已满足人数的报名房间，只接受 POST |
| `/undercover/end` | 结束当前谁是卧底房间或对局，只接受 POST |
...
| `/operator/idle-exit?minutes=30` | 设置闲置退出；触发时自动暂停播放器并关闭目标游戏窗口；`enabled=0` 表示取消 |
...
正式业务接口会返回 `taskId`；组合启动流程返回 `taskIds`。`/monitor.tasks` 可查看等待、执行中、完成、失败和已撤销状态，只有尚未开始的任务允许撤销。远程播放控制、远程点歌、远程 AI 点歌和控制台发言由单一业务运行时保持顺序，再交给正式任务执行运行时完成搜索、AI、播放、出队和游戏内反馈。控制台来源拥有最高权限，不受候选歌曲审核限制。程序必须以管理员权限启动，Web 面板不提供管理员状态或窗口状态诊断。

## 配置说明

程序只读取当前版本的 `config.yaml`，不会自动迁移、改写或备份配置。所有当前配置段和字段都必须显式存在；配置文件不存在、字段缺失、字段未知、类型不匹配或模块校验失败时，会在启动任何运行时、监听端口或游戏输入前直接报错退出。只有自定义工作流步骤中类型明确为可选的参数可以省略。请把与当前发布包匹配的 `config.yaml` 放在程序启动工作目录。

本次清理也收紧了本地持久化队列：只接受包含 `nextId` 和 `items` 的当前 JSON 格式，不再自动转换旧数组、`queue` 包装或 `next_id` 字段。
...
| `friend_delivery` | 好友快速匹配和确认未发送消息的自动重试参数 |
...
| `data/playback-state.json` | 播放器确认状态 |
| `data/hall-state.json` | 大厅倒计时和到期提醒状态 |
