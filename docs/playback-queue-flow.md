...
`@歌词` 查询当前歌词一次；`@歌词 1` 到 `@歌词 10` 会在指定秒数内轮询播放器观测，只发送发生变化且非空的歌词行。无参数命令保持一次性回复。持续歌词命令在现有正式任务执行线程中运行，因此最多占用该线程 10 秒；播放器观测线程仍会继续工作。检测到歌曲 URI 切换后会提前结束，避免把下一首歌曲的歌词混入当前输出。
