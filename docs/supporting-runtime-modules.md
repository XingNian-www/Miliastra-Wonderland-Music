...
`src/interfaces/http/tools.rs` 管理高级控制页的低优先级诊断任务。独立后台线程只在正式待处理队列为空且未执行正式命令时取出任务；失败会写入任务结果，不会中断主循环或触发业务恢复动作。`/tools` 页面提供 OCR、聊天扫描、UI/大厅识别、命名模板匹配、坐标点击、按键、OCR 后端探测与 AI 候选诊断。
...
`src/ui/locator.rs` 负责大厅名称和剩余时间的 OCR 结果整理。
...
`src/features/playback/state.rs` 保存程序运行状态：

- `playback.state`：确认播放状态，例如空闲、启动中、已确认点歌播放、用户暂停、等待队列暂停、外部播放或未知。
- `playback.pauseReason`：暂停原因，区分用户主动暂停和系统等待队列暂停。
- `playback.activeRequest`：当前由本项目确认的活动播放请求，包括关键词、来源、请求 URI、确认 URI、歌名歌手和开始时间。
- `playback.lastObservation`：最近一次播放器观测及可靠性。
- 大厅剩余时间缓存和提醒标记。
...
运行时播放状态、队列、同歌历史和娱乐永久记录都使用同一原子文件写入边界，当前 schema 的未知字段会被拒绝。海龟汤另有一份永久使用记录，默认路径是 `data/turtle-soup-used.json`。它只保存已经被选中的题目 ID，并使用临时文件加替换的方式原子写入。海龟汤当前会话仍是进程内状态，程序重启不会恢复；正式题库 `turtle_soup.yaml` 是只读输入，不属于状态文件。

谁是卧底使用 `data/undercover-used.yaml` 永久记录已经选中的无序词对。程序在第一次发词前原子写入记录，取消或发送失败也不会恢复；记录无法解析时拒绝开局，不会覆盖损坏文件。当前报名房间和对局只保存在内存中。
