...
成语接龙和海龟汤通常是低优先级娱乐例外：聊天扫描线程只更新娱乐状态或投递 AI 请求，游戏内短回复进入独立延迟发送队列，不创建 `PendingTask`。成语 `#解释` 例外地进入正式任务队列，复用 `@帮助` 的独占批量发送路径，避免来源和长解释分段被穿插或部分丢弃。两种玩法由共享娱乐协调器全局互斥。

```mermaid
flowchart TD
    A["主扫描线程<br/>聊天OCR"] -->|普通 ParsedCommand| P["待执行任务队列<br/>VecDeque<PendingTask>"]
    A -->|#接龙 / #海龟汤 / #提问| J["娱乐状态 / AI Worker / 延迟回复队列"]
...
延迟聊天发送线程只在以下条件同时满足时取得输入权：主 `pending` 队列为空、没有命令或屏幕独占 Web 工具正在执行、未暂停，并且目标聊天可用。普通短回复仍使用单条接口；海龟汤分段批次使用可中断的 `ChatOutput::send_batch_interruptible()` 或 `ChatOutput::send_current_chat_batch_interruptible()`。海龟汤的“当前大厅”目标会按取得输入权时的监听驻留选择一级或二级接口。

批量接口复用同一个窗口和输入对象，一级大厅只打开一次聊天。它在每条消息完成后、下一条开始前重新检查程序运行、暂停、正式任务队列、海龟汤会话和监听驻留。需要让行时，一级接口先关闭聊天，发送线程按返回的成功数量移除已发送消息，再把剩余批次放回队首并释放 `CommandExecutingGuard`；二级接口不关闭当前聊天。失败也先提交部分成功进度，只重试首条未发送消息。全部内容已经发出但一级聊天收尾失败时只记录收尾错误，仍按投递成功处理，避免重复发送汤面或汤底。
...
主扫描线程在 `run_scan_loop()` 里截图、检测 UI、扫描聊天。普通命令进入待执行任务队列；成语接龙直接更新纯内存状态，海龟汤命令直接更新会话状态，`#` 提问只进入独立 AI Worker 队列。它们的游戏回复统一走延迟聊天发送线程。

入队前有几层过滤：

1. 空 OCR 结果不处理。
2. `command::parse_entertainment_shortcut()`、`command::parse_text()` 或 `custom_workflow::parse_text()` 必须能解析成命令。
...
4. 如果明确是二级界面，按 ESC 返回上一级。
5. 如果是未知界面，先按 `post_settle_ms` 等待一次并复检；仍未知才按 ESC。
6. 在 `ui_timeout_ms` 内循环。
7. 超时返回 `Ok(false)`。
8. 目标窗口不可用或截图/检测失败则返回错误。
...
返回一级界面本身用 `return_to_primary_by_escape()`。它不会依据首次出现的未知过渡帧立即发送 ESC，而是先等待一次并重新检测；真正按 ESC 后如果连续失败，等待时间会逐步增加，超过 5 次后固定到 2000ms。
...
- 娱乐状态和海龟汤 AI 请求不属于业务任务；延迟发送线程只在主命令完全空闲时取得游戏输入，海龟汤批量接口会在每个分段之间检查并让行。
