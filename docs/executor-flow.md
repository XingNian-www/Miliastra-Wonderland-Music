...
| `TurtleSoupQuestion` | 按观察顺序提交一条已稳定提问；任务标签不含正文。 |
...
普通娱乐命令和稳定海龟汤提问先按观察顺序进入正式任务队列，再由执行器更新 `BusinessRuntime` 状态或提交后续效果；这样后出现的玩法输入不会越过前面的控制命令。命令可用性开关、闲置退出和监听模式控制保留既有运维路径。闲置退出由扫描循环领取到期状态后先调用播放器控制器的用户暂停，再关闭目标游戏窗口；用户暂停原因会阻止播放器监控自动恢复或出队，已排入的自动出队任务在执行入口再次校验暂停状态，软件主进程和播放器运行时继续保留。需要发送的结果仍成为延迟聊天项、`CardGameEffect` 或 `UndercoverEffect`，再取得调度和 UI 顺位。

## 输入顺序与效果通道

正式输入队列确定业务状态转换的先后，效果通道确定外部发送何时可以开始，两者不能合并成一个判断：

- 普通短回复在命令任务内生成，但进入 `DeferredChatQueue`。
- 好友验证、私聊发牌和不可拆分 UI 批次由当前 Formal 任务完整执行。
- 牌局命令按可能产生的完整效果链选择 lane；后续恢复必须保持首个效果的 lane。
- 时间敏感输入携带观察帧 `captured_at`，排队耗时不能改写回合、截止或空闲活动时间。

因此“输入进入 Formal”只承诺因果顺序，不承诺其回复抢占已经存在的正式任务。
...
管理投票工作线程可以取得独占决策读取范围并等待稳定粉字投票，但不能操作 UI。独占范围只隔离投票基线和决策语义，普通聊天观察仍进入共享命令流；普通命令照常经过屏幕锁和待执行队列，投票回复由决策读者消费且不会被自定义工作流抢占。它结束后提交 `ModerationResultTask`；正式任务再调用 `ExecuteModeration`。流程租约和临时一级租约负责防重复与最终释放，结束时不人为制造 `ExclusiveSession` 缺口。
...
- 新的聊天玩法还必须分别决定输入排序与效果通道，不能因为输入经过正式队列就固定使用 Formal 效果。
