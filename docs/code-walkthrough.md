...
主扫描循环只负责把解析出的命令交给 `CommandLockState`。普通命令和谁是卧底命令通过屏幕锁后进入待执行任务队列；成语接龙和海龟汤命令通常直接进入娱乐模块，只有需要稳定批量发送来源和解释的 `@解释` 会进入正式任务队列；`#` 提问不构造 `ParsedCommand`。谁是卧底发词内容、私聊描述和投票会在日志与监控 OCR 中隐藏。
