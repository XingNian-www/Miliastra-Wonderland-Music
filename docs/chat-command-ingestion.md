...
海龟汤控制命令同样由命令信封路由到 `TurtleSoupCommand`。进行中的普通“以 `#` 开头的问题”不是控制命令：它先通过海龟汤模块的稳定提问观察，再作为 `PendingTask::TurtleSoupQuestion` 进入正式输入队列；只有任务轮到执行时才暂存分段或提交 AI。`##编号内容` 按昵称和编号暂存，`##提交` 校验连续编号后合并，`##取消` 清空当前昵称的暂存内容。命令识别关闭时不接收新输入，但现有对局仍继续计时并可自动结算。

收到 `DisableCommands` / `EnableCommands` 后，扫描线程会立即更新 `commands_enabled`，同时命令本身仍会入队执行，以便发送游戏内反馈并写执行日志。

`IdleExit` 是例外：它作为 `AdministrationCommand` 在扫描侧应用即时状态变更并写日志，不进入正式任务队列。计时到期后，扫描循环先通过播放器控制器发送用户暂停，再关闭目标游戏窗口；用户暂停原因会阻止播放器监控自动恢复或自动出队，已排入的自动出队任务在执行入口再次校验暂停状态，软件主进程和播放器运行时继续运行。
