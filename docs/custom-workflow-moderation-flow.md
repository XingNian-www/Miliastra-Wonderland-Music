...
入口是 `execute_invite_with_announce(username)`。

整体规则：

- 如果当前已经是公共大厅，直接通知好友“已同意加入大厅,请注意启动麦克风”，然后执行邀请。
...
真正邀请用户的流程在 `execute_invite(username)` 和 `execute_invite_steps(username)`。

它会打开好友相关界面，通过模板和 OCR 找到目标用户，并按配置区域点击邀请按钮。执行失败时会尝试从临时界面返回一级界面；执行成功后会等待一段兜底时间，再返回一级界面。
