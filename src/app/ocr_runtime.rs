...
use super::ocr::{OcrLine, ResolvedOcrArgs, make_ocr_engine, merge_ocr_lines, recognize_lines};
...
#[derive(Clone)]
pub(super) struct OcrRuntimeHandle {
    channel: Arc<OcrChannel>,
}

impl OcrRuntimeHandle {
    pub(super) fn recognize_lines(
        &self,
        image: DynamicImage,
        priority: OcrPriority,
    ) -> Result<Vec<OcrLine>> {
...
pub(super) struct ProductionOcrDevice {
...
impl ProductionOcrDevice {
    pub(super) fn new(args: ResolvedOcrArgs) -> Result<Self> {
...
impl OcrRuntime {
    pub(super) fn start(device: impl OcrDevice, capacity: usize) -> Result<Self> {
        if capacity == 0 {
            return Err(anyhow!(
    ...
    pub(super) fn handle(&self) -> OcrRuntimeHandle {
        self.handle.clone()
    }
