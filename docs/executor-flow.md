...
成语接龙和海龟汤通常是低优先级娱乐例外：聊天扫描线程把成语命令提交给业务运行时，把海龟汤命令更新到其应用服务或投递 AI 请求，游戏内短回复进入独立延迟发送队列，不创建 `PendingTask`。成语 `#解释` 例外地进入正式任务队列，复用 `@帮助` 的独占批量发送路径，避免来源和长解释分段被穿插或部分丢弃。两种玩法由共享娱乐协调器全局互斥。
...
主扫描线程在 `run_scan_loop()` 里截图、检测 UI、扫描聊天。普通命令进入待执行任务队列；成语接龙命令提交给业务运行时，海龟汤命令更新其应用服务，`#` 提问只进入独立 AI Worker 队列。它们的游戏回复统一走延迟聊天发送线程。
...
- 娱乐状态和海龟汤 AI 请求不属于业务任务；延迟发送线程只在主命令完全空闲时开始取得游戏输入，海龟汤批量接口开始后独占至完成或失败。
