...
| 文件 | 职责 |
| --- | --- |
| `src/config/mod.rs` | 配置结构、配置加载、迁移调用、默认配置内嵌。 |
| `src/config/migration.rs` | 配置迁移规则、字段移动、默认值变更、渲染和未迁移字段注释。 |
...
1. `src/config/mod.rs`：读 `AppConfig::load_or_create()`。
2. `src/config/migration.rs`：读 `migrate_config_text()`。
