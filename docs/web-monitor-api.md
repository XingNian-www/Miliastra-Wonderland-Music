...
少数接口是例外：`/queue/add` 直接写音乐播放队列，`/open-scheme` 直接调用 FeelUOwn 播放 URI，`/state/save` 直接 patch 运行状态。它们不做游戏窗口输入。
...
| `/state/save` | 对运行状态做有限字段 patch 并保存。 |
| `/open-scheme` | 校验 `fuo://` URI 后直接调用 FeelUOwn 播放，并清除暂停标记。 |
...
| 点歌 | `/searchPlay`、`/ai/search`、`/queue/add`、`/search`。 |
...
- `/queue/add`、`/open-scheme`、`/state/save` 是直接修改接口，需要谨慎使用。
