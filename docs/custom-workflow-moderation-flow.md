...
1. 开始等待前扫描一次当前聊天。
2. 用 `DecisionScreenLock` 记录已经存在的 `@确认` 或 `@跳过`。
3. 在超时时间内循环截图并扫描聊天。
...
好友发言入口是 `send_friend_message(username, message)`。它用于邀请结果反馈，也可以被自定义工作流步骤调用。

流程：

1. 先尝试返回一级界面。
2. 按 Enter 打开聊天或好友界面。
3. 在好友列表区域 OCR 查找目标用户名。
4. 点击用户名文本框中心。
5. 等聊天区域像素稳定，认为好友聊天已打开。
6. 调用 `chat_output.send_current_chat(message)` 发送当前聊天。
7. 无论发送是否成功，都尝试从临时界面返回一级界面。

这个流程要求游戏窗口已经处于可输入状态。它不会把好友反馈伪装成玩家命令，也不参与普通聊天命令解析。
...
真正邀请用户的流程在 `execute_invite(username, password)` 和 `execute_invite_steps(username, password)`。

它会打开好友相关界面，通过模板和 OCR 找到目标用户，并按配置区域点击邀请按钮。若邀请命令带有 6 位密码，会在点击“进入大厅”后等待一次邀请步骤延迟，然后逐位键盘输入密码。执行失败时会尝试从临时界面返回一级界面；执行成功后会等待一段兜底时间，返回一级界面，并发送“BOT已经就绪,可以使用@麦克风指令了”。

邀请流程的重点边界是：

- 先做公共大厅判断，避免重复询问。
- 好友反馈失败只记录错误，不会反过来阻塞已经决定的邀请行为。
- 拒绝分支不会执行邀请 UI。
...
状态机失败时返回未执行。无论成功失败，外层都会尝试返回一级界面，并根据结果发送大厅反馈。
...
- 原子动作默认游戏已经聚焦；自定义流程和命令执行准备负责建立前置条件。
- 后台投票线程只等待和入队，不直接操作游戏窗口。
- 好友发言必须先回一级界面，再打开好友聊天。
