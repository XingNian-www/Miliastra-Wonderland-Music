...
fn merge_cookie_header(cookies: &mut BTreeMap<String, String>, value: &str, allowed: &[&str]) {
    for part in value.split(';') {
        ...
        let name = name.trim();
        let raw = raw.trim();
        if allowed.contains(&name) && !raw.is_empty() {
            cookies.insert(name.to_owned(), raw.to_owned());
        }
    }
}
...
fn merge_netease_cookie_value(
    ...
    match value {
        ...
        Value::Object(fields) => {
            for name in allowed {
                if let Some(value) = fields.get(*name).and_then(value_from_cookie)
                    && !value.is_empty()
                {
...
fn parse_netease_status(body: &Value, headers: HeaderMap) -> NeteaseStatus {
    let code = value_i64(body.get("code")).unwrap_or(-1);
    match code {
        801 => NeteaseStatus::Waiting,
        802 => NeteaseStatus::Scanned,
        803 => {
            let mut cookies = parse_set_cookie_headers(&headers, &["MUSIC_U", "__csrf"]);
            for field in ["cookie", "cookies"] {
                if let Some(value) = body.get(field) {
                    merge_netease_cookie_value(&mut cookies, value, &["MUSIC_U", "__csrf"]);
                }
            }
            if cookies
                .get("MUSIC_U")
        ...
        800 => NeteaseStatus::Expired,
        _ => NeteaseStatus::Error("网易云音乐二维码登录状态未知".to_owned()),
    }
}
    ...
    loop {
        ...
        match connection.recv_timeout(Duration::from_secs(1)) {
            Ok(Ok(MqttEvent::Incoming(Packet::Publish(publish)))) => {
                ...
                match event_type {
                    "scanned" => continue,
                    "canceled" => return Ok(QqMqttOutcome::Canceled),
                    "timeout" => return Ok(QqMqttOutcome::Expired),
                    "loginFailed" => return Err("QQ 音乐扫码登录失败".to_owned()),
                    "cookies" => {
...
fn qq_mobile_login(
    ...
    let cookies = payload
        .get("cookies")
        .ok_or_else(|| "QQ 音乐扫码响应缺少 cookies".to_owned())?;
    let uin = qq_cookie_value(cookies, &["qqmusic_uin", "musicid", "uin"])
        .ok_or_else(|| "QQ 音乐扫码响应缺少账号标识".to_owned())?;
    let token = qq_cookie_value(cookies, &["qqmusic_key", "musickey", "key"])
    ...
    let mut result = parse_set_cookie_headers(
        &headers,
        &[
            "uin",
            "wxuin",
...
fn qq_cookie_value(cookies: &Value, names: &[&str]) -> Option<String> {
    match cookies {
        Value::Object(fields) => names.iter().find_map(|name| {
            fields
                .get(*name)
                .and_then(value_from_cookie)
                .filter(|value| !value.is_empty())
        ...
        Value::Array(items) => items.iter().find_map(|item| {
            ...
            names
                .contains(&name)
                .then(|| object.get("value").or_else(|| object.get("val")))
                .flatten()
                .and_then(value_from_cookie)
                .filter(|value| !value.is_empty())
        }),
...
fn merge_qq_login_fields(cookies: &mut BTreeMap<String, String>, body: &Value) {
    let Some(data) = qq_data_object(body) else {
        return;
    };
    for (source, aliases) in [
        ("musicid", &["uin", "wxuin"][..]),
        ("musicuin", &["uin", "wxuin"][..]),
        ("str_musicid", &["uin", "wxuin"][..]),
        ("qqmusic_uin", &["uin", "wxuin"][..]),
        ("musickey", &["qqmusic_key", "qm_keyst", "lqm_keyst"][..]),
        ("qqmusic_key", &["qqmusic_key", "qm_keyst", "lqm_keyst"][..]),
        ("openid", &["psrf_qqopenid", "openid", "wxopenid"][..]),
        ("wxopenid", &["psrf_qqopenid", "openid", "wxopenid"][..]),
        (
            "access_token",
        ...
        ("refreshKey", &["psrf_qqrefresh_key", "refresh_key"][..]),
        ("unionid", &["psrf_qqunionid", "unionid", "wxunionid"][..]),
        ("wxunionid", &["psrf_qqunionid", "unionid", "wxunionid"][..]),
    ] {
        if let Some(value) = data.get(source).and_then(value_from_cookie) {
...
fn qq_data_object(value: &Value) -> Option<&serde_json::Map<String, Value>> {
    if let Some(data) = value.get("data").and_then(Value::as_object) {
        return Some(data);
    }
    let object = value.as_object()?;
    for key in ["req_0", "req", "req_1", "req1"] {
...
fn qq_business_code(value: &Value) -> Option<i64> {
    let top_level_code = value_i64(value.get("code"));
    if top_level_code.is_some_and(|code| code != 0) {
        return top_level_code;
    }
    let mut nested_success = false;
    for key in ["req_0", "req", "req_1", "req1"] {
        if let Some(code) = value
            .get(key)
            .and_then(|value| value_i64(value.get("code")))
        {
            if code != 0 {
                return Some(code);
            }
    ...
    top_level_code.or_else(|| nested_success.then_some(0))
}
...
fn normalize_qq_aliases(cookies: &mut BTreeMap<String, String>) {
    for aliases in [
        ...
        let value = aliases.iter().find_map(|name| {
            cookies
                .get(*name)
                .filter(|value| !value.is_empty())
                .cloned()
