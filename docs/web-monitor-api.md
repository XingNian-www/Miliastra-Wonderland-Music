...
少数接口不创建正式 UI 任务：`/queue/add`、`/player/play-uri` 和 `/state/save` 通过 `HttpTaskPort` 提交播放模块 mutation。它们仍由业务运行时串行修改队列或状态；`/player/play-uri` 不直连 FeelUOwn，也不把播放器标记为外部播放。
...
| `/state/save` | 提交有限字段运行状态 patch，由播放模块所有者校验并保存。 |
