...
const CHANGED_DEFAULT_FIELDS: &[ChangedDefaultField] = &[
    ...
    ChangedDefaultField {
        path: "invite.friend_name_stable_count",
        old_default: 2,
        changed_in_version: 27,
    },
    ChangedDefaultField {
        path: "turtle_soup.nickname_stable_count",
        old_default: 2,
        changed_in_version: 27,
    },
    ChangedDefaultField {
        path: "turtle_soup.content_stable_count",
        old_default: 2,
        changed_in_version: 27,
    },
];
...
const MOVED_FIELDS: &[(&str, &str)] = &[
    ...
    (
        "undercover.nickname_stable_count",
...
"#;
    ...
    #[test]
    fn migrates_v23_to_disabled_undercover_defaults() {
        let old = "config_version: 23\n";
        let report = migrate_config_text(old, include_str!("../../config.yaml"))
            .expect("migration succeeds")
            .expect("migration needed");
        let migrated: Value = serde_yaml::from_str(&report.text).expect("valid migrated yaml");
    ...
    #[test]
    fn migrates_v24_to_global_friend_name_stability_default() {
        ...
        assert_eq!(
            get_path(&migrated, &["invite", "friend_name_stable_count"]).and_then(Value::as_u64),
            Some(0)
        );
    }

    #[test]
    fn migrates_v25_custom_undercover_nickname_stability_to_global_value() {
        let old = "config_version: 25\nundercover:\n  enabled: true\n  nickname_stable_count: 4\n";
        let report = migrate_config_text(old, include_str!("../../config.yaml"))
            .expect("migration succeeds")
            .expect("migration needed");
        let migrated: Value = serde_yaml::from_str(&report.text).expect("valid migrated yaml");
    ...
    #[test]
    fn migrates_v26_stability_defaults_to_global_inheritance() {
        let old = r#"config_version: 26
invite:
  friend_name_stable_count: 2
turtle_soup:
...
"#;
        ...
        for path in [
            ...
            assert_eq!(get_path(&migrated, path).and_then(Value::as_u64), Some(0));
        }
    }
