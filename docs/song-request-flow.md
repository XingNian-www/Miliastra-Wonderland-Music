...
待执行任务队列在入队时使用 `FormalTaskDedupKey` 对仍处于 `queued` 状态的同语义任务去重；它只防止同一业务动作重复排队，不会把已经运行或已经完成的任务重新判重，也不会替代音乐播放队列和长时间同歌历史的歌曲去重。
...
- `/player/play-uri` 同样是控制台高权限的音乐播放队列 mutation：它保存已知 `fuo://` URI，等待正式出队时按 URI 播放，不直接调用 FeelUOwn，也不绕过播放器确认。
