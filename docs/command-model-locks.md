...
确认、跳过、换源、邀请确认/拒绝和管理投票回复属于保留决策语法。即使自定义工作流声明了同名触发词，路由器也不会把它们转成普通命令；决策读者仍按自己的 `DecisionScreenLock` 消费。
...
决策读者独占的是决策消息基线和读取条件，不是共享聊天流。等待期间的普通聊天帧继续由一级或二级监听发布和解析：一级先经过同轮合并与命令屏幕锁，二级先经过气泡序列/消息基线保护；两者都再经过待执行范围去重后进入正式队列。会话结束不再人为产生 `ExclusiveSession` 缺口。
...
- 独占决策等待不能暂停普通命令流，也不能让同一普通命令绕过屏幕锁或待执行去重。
