...
谁是卧底默认关闭，支持 4-11 人单卧底和 6-11 人双卧底。创建房间和开始对局使用公屏，其他玩家只能通过好友私聊报名；机器人通过好友私聊发送位置和词语，描述使用公屏 `#内容`，投票使用好友私聊 `#A`。词对会在发词前永久标记为已用，Web 只提供脱敏状态、开始当前房间和结束控制。完整玩法见 [谁是卧底](docs/undercover.md)。
...
同意或默认同意时，机器人会把发起者昵称、私聊反馈、可选密码和最终驻留目标一次提交给完整邀请事务。该事务在同一次 UI 独占期间确认唯一好友、发送反馈、进入邀请页面、执行邀请并恢复监听驻留；反馈失败会作为警告保留并继续邀请，已经确认进入大厅的邀请不会因收尾恢复失败而重放。拒绝邀请使用普通好友投递事务发送反馈。
