...
谁是卧底默认关闭，支持 4-11 人单卧底和 6-11 人双卧底。玩家通过大厅报名，机器人通过好友私聊发送位置和词语；描述和投票也只接受好友私聊。词对会在发词前永久标记为已用，Web 只提供脱敏状态、开始当前房间和结束控制。完整玩法见 [谁是卧底](docs/undercover.md)。
...
程序只读取当前版本的 `config.yaml`，不会自动迁移、改写或备份配置。配置文件不存在、必填字段缺失、字段未知或类型不匹配时会直接报错退出；标注为可选的配置段和带默认值的字段可以省略。请把与当前发布包匹配的 `config.yaml` 放在程序启动工作目录。
