...
- 娱乐互斥、成语接龙状态、斗地主牌局、海龟汤会话、按昵称隔离的编号答案草稿和低优先级分段回复队列。
...
主扫描循环只负责把解析出的命令交给 `CommandLockState`。普通命令通过屏幕锁后进入待执行任务队列；成语接龙和海龟汤命令通常在屏幕锁后直接进入娱乐模块，只有需要稳定批量发送来源和解释的 `@解释` 会进入正式任务队列；`#` 提问不构造 `ParsedCommand`。
